var fs = require('fs'); //FileSystem module whitch allows us to read and write local storage
var rawData = fs.readFileSync('./put your .json file here/json.json'); //reads .json in rawdata (buffer data)
var objData = JSON.parse(rawData); //converting raw data from buffer to object
var a = '<root>\n'; //string to store whole conversion to .xml
for (var i in objData) { //going through every property of root object (if there is more than one property)
    ...
    if (objData[i].constructor == Array) { // checking if root element is array
        for (var b in objData[i]) {
            if (typeof objData[i][b] === 'undefined' || objData[i][b] === null) { //changing property of null or undefied with " " if property is null, for root element
                ...
                for (var j = 0; j < objData[i][b].length; j++) {
...
fs.writeFile("./yourXmlFile.xml", a, function (err) {
